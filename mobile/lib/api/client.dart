    ...
    return AsrStatus(
      ...
      mobilePracticeSkillEvidence: json.containsKey('mobile_practice_skill_evidence')
          ...
          : (json.containsKey('practice_skill_evidence')
              ...
              : true),
      ttsSpeakNorm: '${json['tts_speak_norm'] ?? 'v5'}',
...
class AsrClient {
  ...
  /// DELETE /api/cache/papers/{id} — local + GCS paper + user records (design/102+177).
  Future<void> deletePaper(String cacheId, {String? handoffId}) async {
    final id = cacheId.trim();
    if (id.isEmpty) {
      throw AsrApiException('cache id is empty', 400);
    }
    final headers = await _headers();
    final hid = (handoffId ?? '').trim();
    if (hid.isNotEmpty) {
      headers['X-Asr-Handoff-Id'] = hid;
    }
    try {
      final res = await _http
          .delete(
      ...
      // design/177 — make DELETE timeouts joinable (route=cache/delete).
      _breadcrumbTimeout('cache/delete', e);
      rethrow;
    }
  }
...
throw AsrApiException(
  ...
  Future<SpokenTextResult?> fetchSpokenText(String text) async {
    ...
    return SpokenTextResult(
      spoken: spoken,
      speakNormVersion: '${map['speak_norm_version'] ?? 'v5'}',
    );
  }
