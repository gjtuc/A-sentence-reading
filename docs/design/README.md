...
73는 업로드·ingest 호출 횟수 한도 (0.3.1).  
76은 프로세스 종료 후 WorkManager 이어올리기 (0.3.1).  
77은 이메일 매직링크 로그인 · 앱 딥링크 (0.3.1).
78은 이메일 비밀번호 가입/로그인 제거 (0.3.1).
79는 쉐도잉 연습 옵트인 킬·설정 토글 (0.3.1).
80 — 쉐도잉 청크 계획(유저별·ingest/백필) (0.3.1).
81 — 헤더 ⋯ overflow · CD 쉐도잉 킬 (0.3.1).
82 — 쉐도잉 연습 모드(루프·유저별 takes) (0.3.1).

음절(30)은 **하지 않음** (원복).
83은 로그인 강제 게이트(웹·API·앱) (0.3.1).
84은 로그인 후 초대 대기 전용 셸 (0.3.1).
