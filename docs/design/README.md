...
73는 업로드·ingest 호출 횟수 한도 (0.2.98).  
76은 프로세스 종료 후 WorkManager 이어올리기 (0.2.98).  
77은 이메일 매직링크 로그인 · 앱 딥링크 (0.2.98).
78은 이메일 비밀번호 가입/로그인 제거 (0.2.98).
79는 쉐도잉 연습 옵트인 킬·설정 토글 (0.2.98).
80 — 쉐도잉 청크 계획(유저별·ingest/백필) (0.2.98).
81 — 헤더 ⋯ overflow · CD 쉐도잉 킬 (0.2.98).
