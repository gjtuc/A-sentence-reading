...
> **제품 기본 (design/142, 0.3.58):** 키보드 듣고 적기는 **끔**. Enter로 오버레이를 열지 않는다.  
> 응급 복구만 `ASR_SENTENCE_NOTES_KEYBOARD=1`. 아래는 복구·옛 동작 명세.

## 제품 결정

- **Enter** (노트 닫힌 상태 · 플래그 on일 때만) → 쓰기 오버레이 + **현재 문장 TTS 자동 재생**
...
- 프롬프트(고정): *이 문장에서 알아차린 것을, 여러 사람에게 새로 들려준다고 생각하고, 꺼내보고 꺼내보고 꺼내보세요.*
- 노트 열린 채 · **입력칸에 커서가 없을 때** `←`/`→` = 저장 후 이전/다음 문장 · **Space** = TTS
- **Esc** = 노트 닫기 (브라우저 전체화면보다 우선 · `keyboard.lock(Escape)` + 필요 시 FS 재진입)
- **시트 밖 배경 클릭** = 저장 후 닫기
