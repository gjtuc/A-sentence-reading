...
def ensure_server() -> int:
    ...
    _log(f"START: launching uvicorn via {sys.executable}")
    ...
    if server_up():
        ...
        return 0
    _log("FAIL: server did not become ready")
    return 1
...
def register_task(*, quiet: bool = False) -> int:
    """현재 파이썬으로 ensure를 돌리는 작업 스케줄러 작업을 등록한다."""
    if sys.platform != "win32":
        if not quiet:
