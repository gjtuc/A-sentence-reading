"""
Azure Document Intelligence prebuilt-layout → Figure[] (design/147 · 149).
...
design/149 — filter Azure body captions; composite fig+caption clip PNG.
"""
...
def _match_fig_caption_with_rect(page, rect) -> tuple[str, object | None]:
    ...
    for _key, caption, cap_rect in _labeled_caption_hits(
        ...
        if mid < float(rect.x0) - 24 or mid > float(rect.x1) + 24:
            continue
        gap = float(cap_rect.y0) - float(rect.y1)
        if -8 <= gap <= 160:
            dist = abs(int(gap * 100))
            if dist < best[2]:
        ...
        for figure in result.figures or []:
            ...
            if rect is not None:
                ...
                if cap_rect is not None:
                    clip = rect | cap_rect
                png = _render_page_clip(page, clip) or b""
            if not png:
                png = azure_png
