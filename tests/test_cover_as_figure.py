"""design/135 — title-page cover as carousel figure 1 (0.3.53 · rich-v13)."""
...
def test_status_and_docs_pin_cover_chip():
    ...
    assert st["pipeline_version"] == "rich-v13"
    assert st["cover_as_figure"] is True
    assert st["mobile_cover_as_figure"] is True
    assert PIPELINE_VERSION == "rich-v13"
    assert "rich-v13" in TYPO.read_text(encoding="utf-8")
    assert "0.3.53" in PUB.read_text(encoding="utf-8")
